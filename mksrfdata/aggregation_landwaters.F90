...
!$OMP PRIVATE(n_landwaters_patches,area_landwaters_patches) &
!$OMP PRIVATE(f_landwaters,area_for_sum,isfirst)
#endif
      do j = 1, lat_points
         ...
         do i = 1, lon_points
          ...
          ! [Lake (1)] -> [reservoir (2)] -> [river (3)]
            if(np == 0)then
               ...
               fraction_landwaters_patches(:,i,j) = 0.0
               nn = n_landwaters_patches(1)
               if(nn>=1 .and. nn<=3) fraction_landwaters_patches(nn,i,j) = 1.0
            else
               f_landwaters(:) = 0.
               do n = 1, np
               ...
               enddo
               fraction_landwaters_patches(:,i,j) = f_landwaters(:) / sum(area_landwaters_patches(1:np))
            endif
