...
!----------weinan------------
      ...
      integer(kind=1), allocatable :: int_soil_grav_l (:,:) ! gravel
...
!----------------------------
      ...
      real(r8),allocatable :: ydatc(:,:)
      real(r8),allocatable :: ydatv(:,:)
      real(r8),allocatable :: ydatb(:,:)
...
! ........................................
      ...
      allocate ( fjacc                   (1:n_fine_gridcell,     nc) )
      allocate ( fjacv                   (1:n_fine_gridcell,     nv) )
      allocate ( fjacb                   (1:n_fine_gridcell,     nb) )
      allocate ( fvecc                   (1:n_fine_gridcell)         )
      allocate ( fvecv                   (1:n_fine_gridcell)         )
      allocate ( fvecb                   (1:n_fine_gridcell)         )
#endif
...
! (24) Read in gravel (% volume)
         ...
         open(iunit,file=trim(lndname),access='direct',recl=length,form='unformatted',status='old')
         do nrow = nrow_start, nrow_end
            read(iunit,rec=nrow,err=100) int_soil_grav_l(:,nrow)
         enddo
         close (iunit)
#endif
!----------------------------------------------------------------
...
#if(defined Gravels0)
!                  soil_grav_l(nsl) = int_soil_sand_l(ncol_mod,nrow) ! Volumetric in %
                  soil_grav_l(nsl) = 0.0
                  ...
                  if (soil_bd_l(nsl) < 0.111 .or. soil_bd_l(nsl) > 1.699 .or. soil_oc_l(nsl) > 10.0) then
                     SOM=1.724*soil_oc_l(nsl)
                     soil_bd_l(nsl) = 0.111*1.699/(1.699*SOM/100.+0.111*(100.-SOM)/100.)
                  end if
#endif
#if(defined Naqu || defined Gravels0)
