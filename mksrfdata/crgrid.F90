...
!    edgen: northern edge of grid : > -90 and <= 90 (degrees) (=-90  only for single point)
!    edgee: eastern edge of grid  : > western edge and <= 180 (=-180 only for single point)
!    edges: southern edge of grid : >= -90  and <  90         (=90   only for single point)
!    edgew: western edge of grid  : >= -180 and < 180         (cannot be equal to 180)
!
...
! local variables:
    ...
    ! ------------------------------------------------------------------
      ...
      if (edgew > edgee) then
          dx = (edgee+360.0-edgew)/lon_points       ! the region across the dateline
      else
      ...
      do j = 1, lat_points
         do i = 1, lon_points
            ...
            if(longxy(i,j) - 180.0d0 > -1.0e-8) longxy(i,j) = longxy(i,j) - 360.0
         end do
      end do
    ...
    ! longitudes
    ! if western edge of a gridcell is on Dateline, lonw(i)=-180.
      lonw(1) = edgew
      lone(lon_points) = edgee
      do i = 2, lon_points
         if (longxy(i-1,1) < lonw(i-1))then
             dx2 = longxy(i-1,1) - lonw(i-1) + 360.0
         else
             dx2 = longxy(i-1,1) - lonw(i-1)
         end if
         lone(i-1) = longxy(i-1,1) + dx2
         if(lone(i-1) > 180.0d0)then
            lone(i-1) = lone(i-1) - 360.0
         end if
         lonw(i) = lone(i-1)
         if(abs(lonw(i) - 180.0d0) < 1.0e-8)then
            lonw(i) = -180.0
         end if
      end do
