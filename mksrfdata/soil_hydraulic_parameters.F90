SUBROUTINE soil_hydraulic_parameters(BD,SAND,CLAY,SOC,SOILDEPTH,&
           vf_gravels_s,phi,CampBC_psi_s,CampBC_lambda_s,k_s,&
...
!
! Yongjiu Dai, 12/2013, 06/2018, 01/2019
! ----------------------------------------------------------------
use precision
...
!* real(r8), intent(out) :: VGM_k_s     ! saturated hydraulic conductivity (defined at h=0)

real(r8) SOM, TOPSOIL, BD_om, BD_minerals,a
...
! ---------------------------------------------------------------------
! Porosity of FINE EARTH
! ---------------------------------------------------------------------
!      BD_om = 1.3
!      BD_minerals = 2.65
!      phi = 1.0 - BD*((1.0-SOM/100.0)/BD_minerals + SOM/100.0/BD_om)

!*    CALL thetas(BD,SOC,sand,clay,soildepth,vf_pores_fine_earth)
...
!========================================================
! SOIL water retention curve of Brooks and Corey (1964) 
! simplified by Campbell (1974) with assumption of "zero residual saturation"
! Parameters of the soil water retention curve of FINE EARTH
!
! (1) Dai et al., 2013, Development of a China Dataset of Soil 
!     Hydraulic Parameters Using Pedotransfer Functions for Land Surface Modeling. 
!     J. of Hydrometeorology, 14: 869-887. DOI: 10.1175/JHM-D-12-0149.1
...
! ------------------------------------------
      ...
      call lmder ( SW_CB_dist, m, n, x, fvec, fjac, ldfjac, ftol, xtol, gtol, maxfev, &
                   diag, mode, factor, nprint, info, nfev, njev, ipvt, qtf,&
                   xdat, npoint, ydatc, nc, phi, isiter)
...
! ------------------------------------------
      ...
      call lmder ( SW_VG_dist, m, n1, x, fvec, fjac, ldfjac, ftol, xtol, gtol, maxfev, &
                   diag, mode, factor, nprint, info, nfev, njev, ipvt, qtf, &
                   xdat, npoint, ydatv, nv, phi, isiter )
