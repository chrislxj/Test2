 ...
 subroutine newsnow (itypwat,maxsnl,deltim,t_grnd,pg_rain,pg_snow,bifall,& 
...
!-----------------------------------------------------------------------
      ...
      if(itypwat==2 .AND. t_grnd>tfrz)then  ! snowfall on warmer wetland
...
!         delete by Nan Wei, will consider t_precip in land surface energy budget
!         t_soisno(lb) = tfrz &
