...
!-----------------------------------------------------------------------
  ...
  subroutine  leaftemone (deltim ,csoilc ,dewmx  ,htvp   ,lai    ,&
              ...
              tstar   ,fm        ,fh     ,fq     ,qintr_rain     ,&
              qintr_snow         ,t_precip,  hprl) 
...
! 21/07/2017, Nan Wei:  interaction btw prec and canopy
!=======================================================================
...
!-----------------------------------------------------------------------
        ...
        dtl(it) = (sabv + irab - fsenl - hvap*fevpl + cpliq * &
                   qintr_rain*(t_precip-tl) + cpice * &
                   qintr_snow*(t_precip-tl)) &
            / ((lai+sai)*clai/deltim - dirab_dtl + fsenl_dtl + hvap*fevpl_dtl + cpliq * &
               qintr_rain + cpice * qintr_snow)
        dtl_noadj = dtl(it)
...
#if(defined CLMDEBUG)
      if(abs(err) .gt. .2) &
      write(6,*) 'energy imbalance in leaftemone.F90',it-1,err,sabv,irab,fsenl,hvap*fevpl,hprl
#endif
...
!----------------------------------------------------------------------         
  ...
  subroutine leaftemtwo (deltim ,csoilc  ,dewmx  ,htvp   ,lai    ,&
             ...
             fm      ,fh        ,fq  ,qintr_rain ,qintr_snow ,t_precip,hprl)

!=======================================================================
! Original author : Yongjiu Dai, August 15, 2001
!
! Foliage energy conservation is given by foliage energy budget equation
!                   sunlit:  [Rnet - Hf - LEf + Hpr] = 0
!                   shaded:  [Rnet - Hf - LEf + Hpr] = 0
! The equation is solved by Newton-Raphson iteration, in which this iteration
! includes the calculation of the photosynthesis and stomatal resistance, and the
! integration of turbulent flux profiles. The sensible and latent heat
! transfer between foliage and atmosphere and ground is linked by the equations:
...
!=======================================================================
  ...
  use precision
  use PhysicalConstants, only : vonkar, grav, hvap, cpair, stefnc,cpice,cpliq
  use FRICTION_VELOCITY
  use ASSIM_STOMATA_conductance
  implicit none
...
! functions and their derivatives with respect to temperatures
          ftsun = sabvsun + irabsun - senlsun - hvap*evplsun + cpliq * &
                  qintr_rain*(t_precip-tlsun) * fsun + cpice * qintr_snow*(t_precip-tlsun) * fsun
          ftsha = sabvsha + irabsha - senlsha - hvap*evplsha + cpliq * &
                  qintr_rain*(t_precip-tlsha) * fsha + cpice * qintr_snow*(t_precip-tlsha) * fsha

          dftsunDTlsun = dirabsun_dtlsun - senlsun_dtlsun - hvap*evplsun_dtlsun &
                         - cpliq * qintr_rain * fsun - cpice * qintr_snow * fsun
          ...
          dftshaDTlsha = dirabsha_dtlsha - senlsha_dtlsha - hvap*evplsha_dtlsha &
                         - cpliq * qintr_rain * fsha - cpice * qintr_snow * fsha
...
!-----------------------------------------------------------------------
       ...
       hprl = cpliq * qintr_rain*(t_precip-tlsun) * fsun + cpice * qintr_snow*(t_precip-tlsun) * fsun&
            + cpliq * qintr_rain*(t_precip-tlsha) * fsha + cpice * qintr_snow*(t_precip-tlsha) * fsha
!-----------------------------------------------------------------------
! Derivative of soil energy flux with respect to soil temperature (cgrnd)
!-----------------------------------------------------------------------
       cgrnds = cgrnds + cpair*rhoair*cgh*(1.-wtg0)
...
!=======================================================================
 ...
 implicit none
  ...
  real(r8), INTENT(in) :: ldew   ! depth of water on foliage [kg/m2]
