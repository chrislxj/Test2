...
! -------edit by zsg---------      
    real(r8) :: b_hr = 3.22      ! emprirical constant     
    real(r8) :: smp50 = -102000. ! soil water potential where conductance is reduced by 50%(mm)
    real(r8) :: crt = 0.097      ! cm/(MPa*h), default value 0.097 (Ryel et al., 2002)
    real(r8) :: c_hr(1:nl_soil)  ! relstive soil-root conductance for water(from van Genuchten)
    real(r8) :: rootfr_x         ! root fraction used in HR calculation 
    real(r8) :: stime            ! swich of HR
    real(r8) :: qhr(nl_soil)     ! water change flux due to HR in soil layer nl_soil  
    real(r8) :: qhr_flux(nl_soil,nl_soil) ! water flux due to HR bewteen two soil layers 
    real(r8) :: vol_wilt(nl_soil) ! soil water per unit volume at wilting point
    real(r8) :: smpmax            ! wilting point potential in mm
    real(r8) :: dwat_hr (1:nl_soil)! change of soil water by HR[m3/m3]
! -------end of edition------
!-----------------------------------------------------------------------
...
! -------edit by zsg---------      
    ...
    if(etr > (10.0/2.5104e6))  then
! weak transpiration, 2.5104e6 is latent heat of evap for water 
! it will transfer w/m2 into mm/s 
      stime=0.0
    else
      stime=1.0
    endif
    ...
    do j = 1, nl_soil
    	...
    	c_hr(j) = 1./(1+(smp(j)/smp50)**b_hr)

    enddo

! qHR = 0 at top layers, as Lee et al., 2005 did
    qhr(:) = 0.    
    qhr_flux(:,:) = 0.
    ...
    do k = nl_soil, 3, -1
      do j = k-1, 2, -1
        ...
        if(vol_liq(k)>vol_liq(j)) then
          rootfr_x = rootfr(k)
        else
          rootfr_x = rootfr(j)
        endif
        
        qhr_flux(k,j) = crt*(smp(k)-smp(j))*max(c_hr(k),c_hr(j)) &
                       *(rootfr(k)*rootfr(j))/(1-rootfr_x)*stime 

        qhr_flux(j,k) = -1.0 * qhr_flux(k,j)
        
        ! assume water do not leave the layer which potential is below wilting point through HR
        if(smp(k) < -1.5e5 .and. qhr_flux(k,j) > 0.) then 
           qhr_flux(k,j) = 0.
           qhr_flux(j,k) = 0.
        endif
    ...
    do k = 2, nl_soil
      ...
      qhr(k) = qhr(k)+qhr_flux(j,k)/(3600.*10200.)   ! mm/s
    ...
    vol_wilt(:) = 0.
    do k = 1, nl_soil
       vol_wilt(k) = porsl(k)*(smpmax/psi0(k))**(-1./bsw(k)) !psi0 is nagetive,not like phi0 in old version
    enddo
    ...
    do k = 2, nl_soil
      ...
      dwat_hr(k) = qhr(k)*deltim/dzmm(k)
    
    enddo    
     
    do k = 2, nl_soil       
       dwat(k) = dwat(k) + dwat_hr(k)
    enddo 
! -------end of edition------
