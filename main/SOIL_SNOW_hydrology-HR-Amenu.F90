...
!-----------------------------------------------------------------------
  ...
  subroutine WATER (itypwat,lb,nl_soil,deltim ,&
             ...
             qcharge     ,ipatch      ,rootfr,dlat,ivt) !add rootfr by zsg 
...
!-----------------------Argument---------- ------------------------------
  ...
  real(r8), INTENT(in) :: &
        ...
        rootfr(nl_soil)  , &! fraction of roots in each soil layer --add by zsg
        ...
        dlat          ! latitude in radians----add by zsg
  ...
  integer, INTENT(in) :: &
        ivt        , &! land cover type of USGS classification or others---zsg
        ipatch              ! patch index
!                    
!-----------------------Local Variables------------------------------
!                   
...
!=======================================================================
      ...
      call soilwater(nl_soil,deltim,wimp,smpmin,&
                     ...
                     zwt,dwat,qcharge,rootfr,dlat,ivt) !add rootfr by zsg
...
!-----------------------------------------------------------------------
    ...
    IMPLICIT NONE
    integer, INTENT(in) :: ivt      ! land cover type of USGS classification or others--zsg
    integer, INTENT(in) :: nl_soil  ! number of soil layers
    ...
    real(r8), INTENT(in) :: rootfr (nl_soil)    ! fraction of roots in each soil layer --add by zsg
    real(r8), INTENT(in) :: zwt                ! the depth from ground (soil) surface to water table [m]

    real(r8), INTENT(in) :: dlat          ! latitude in radians--add by zsg
...
! -------edit by zsg---------      
   real(r8) :: &
       amx_hr(1:nl_soil),    &! "a" left off diagonal of tridiagonal matrix
       bmx_hr(1:nl_soil),    &! "b" diagonal column for tridiagonal matrix
       cmx_hr(1:nl_soil),    &! "c" right off diagonal tridiagonal matrix
       rmx_hr(1:nl_soil)      ! "r" forcing term of tridiagonal matrix
   real(r8) :: rpp(1:nl_soil)    ! root pressure potential                       [mm]  
   real(r8) :: krad(1:nl_soil)   ! radial conductivity of the root               [mm/s]
   real(r8) :: kax(1:nl_soil)    ! axial conductivity of the root                [mm/s]
   real(r8) :: vol_wilt(nl_soil) ! soil water per unit volume at wilting point
   real(r8) :: smpmax            ! wilting point potential in mm
   real(r8) :: K_rad = 0.5e-7    ! radial conductivity of the root system        [s^-1]
   real(r8) :: K_axs = 2.0e-1    ! axial specific conductivity of the root system[mm/s]
   real(r8) :: den1              ! used in calculating HR(Amenu model)
   real(r8) :: den2              ! used in calculating HR(Amenu model)
   real(r8) :: den_AHR              ! used in calculating HR(Amenu model)
! -------end of edition------
!-----------------------------------------------------------------------
...
! Method II: 
    ...
    vol_wilt(:) = 0.
    do k = 1, nl_soil
       vol_wilt(k) = porsl(k)*(smpmax/psi0(k))**(-1./bsw(k))
    enddo
    do k = 1, nl_soil
        if (vol_liq(k) <= vol_wilt(k) ) then
            krad(k) = 0
        else
        ...
        ! area of roots in that layer. Thus, 
            krad(k) = rootfr(k)*K_rad*vol_liq(k)/eff_porosity(k)
        end if
        ...
        kax(k) = (rootfr(k)/dzmm(k))*K_axs*0.6
    end do



    
! ROOT PRESSURE POTENTIAL --> HR Model (Amenu et al., 2008)
    ! For the top soil layer
    j      = 1
    den_AHR    = zmm(j+1) - zmm(j)
    amx_hr(j) = 0
    bmx_hr(j) = kax(j)/den_AHR + krad(j)
    cmx_hr(j) = -kax(j)/den_AHR
    rmx_hr(j) = krad(j)*smp(j) - etr - kax(j)
!
    ...
    call tridia (nl_soil ,amx_hr ,bmx_hr ,cmx_hr ,rmx_hr ,rpp) 

    ! ------- end of addtion ---- 

    ! ------- edit by zsg -------
    if(etr > (10.0/2.5104e6)) then !.or.abs(dlat) >0.523)  then !strong transpiration,no HR;middle and high latitude,no HR
...
!      endif
    ...
    else ! weak transipiration,HR actived
       ! Set up r, a, b, and c vectors for tridiagonal solution
       ...
       bmx(j) = dzmm(j)/deltim + dqodw1(j)
       cmx(j) = dqodw2(j)
       rmx(j) = qin(j) - qout(j)     !- krad(j)*(smp(j)-rpp(j)) 
